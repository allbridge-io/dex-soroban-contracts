...
impl TwoPoolSnapshot {
    ...
    pub fn take(testing_env: &TwoPoolTestingEnv) -> TwoPoolSnapshot {
        ...
        let acc_reward_yusd_per_share_p = pool_info.acc_rewards_per_share_p.0.get_unchecked(0);
        let acc_reward_yaro_per_share_p = pool_info.acc_rewards_per_share_p.0.get_unchecked(1);

        let admin_yusd_fee_rewards = pool_info.admin_fee_amount.0.get_unchecked(0);
        let admin_yaro_fee_rewards = pool_info.admin_fee_amount.0.get_unchecked(1);
